...
def test_cli_info_returns_path_and_version(monkeypatch) -> None:
    runner = _runner(Path("C:\\tools\\codex.cmd"))

    def fake_run(args, **kwargs):
        assert args == ["C:\\tools\\codex.cmd", "--version"]
        return _FakeProc(0, b"codex-cli 0.147.0\n")

    monkeypatch.setattr("control_plane.codex_runner.subprocess.run", fake_run)
    path, version = runner.cli_info()
    assert path == Path("C:\\tools\\codex.cmd")
...
def test_check_cli_detects_version_change(monkeypatch, tmp_path) -> None:
    ...
    store.set_setting("codex:cli_version", "codex-cli 0.146.0")
    runner = _runner(Path("C:\\tools\\codex.cmd"))
    runner.attach_store(store)
    monkeypatch.setattr(
        "control_plane.codex_runner.subprocess.run",
        lambda args, **kwargs: _FakeProc(0, b"codex-cli 0.147.0\n"),
    )
    path, version, changed = runner.check_cli()
    assert changed is True
    assert version == "codex-cli 0.147.0"
    assert store.get_setting("codex:cli_version") == "codex-cli 0.147.0"
    assert store.get_setting("codex:cli_path") == "C:\\tools\\codex.cmd"
    store.close()
...
def test_check_cli_no_change_on_same_version(monkeypatch, tmp_path) -> None:
    ...
    store = Store(tmp_path / "cp.db")
    store.set_setting("codex:cli_version", "codex-cli 0.147.0")
    runner = _runner(Path("C:\\tools\\codex.cmd"))
    runner.attach_store(store)
    monkeypatch.setattr(
        "control_plane.codex_runner.subprocess.run",
